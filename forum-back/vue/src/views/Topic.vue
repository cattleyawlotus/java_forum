<template>
  <div style="padding:10px">
    ...
    <!--    搜索区域-->
    <div style="margin: 10px 0">
      <el-input v-model="search" placeholder="请输入标题关键词" style="width: 20%" clearable/>
    ...
    <el-table :data="tableData" border stripe style="width: 100%">
      ...
      <el-table-column prop="category" label="专栏"/>
...
export default {
  ...
  methods: {
    ...
    save() {
      if (this.form.id) { //更新
        request.put("topic", this.form).then(res => {
          console.log(res);
          if (res.code == '0') {
            this.$message({
              type: "success",
        ...
        request.post("/topic", this.form).then(res => {
          console.log(res);
          if (res.code == '0') {
            this.$message({
              type: "success",
    ...
    handleDelete(id) {
      console.log(id);
      request.delete("/topic/" + id).then(res=>{
        if (res.code == '0') {
          this.$message({
            type: "success",
            message: "删除成功"
